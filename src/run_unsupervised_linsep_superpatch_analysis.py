...
from unsupervised_utils import get_topn_aligning_clusters_per_concept, get_most_aligning_cluster_per_concept, \
    ...
    compute_detection_metrics_over_percentiles_allpairs, compute_signed_distances_streaming_multi
...
if __name__ == "__main__":
    for DATASET_NAME in DATASETS:
        for (MODEL_NAME, INPUT_IMAGE_SIZE, SAMPLE_TYPE) in MODELS:
            ...
            CON_LABEL= f"{MODEL_NAME}_kmeans_{N_CLUSTERS}_linsep_patch_embeddings_kmeans_BD_{BALANCE_DATA}_BN_{BALANCE_NEGATIVES}_percentthrumodel_{PERCENT_THRU_MODEL}"

            EMBEDDINGS_FILE = f'{MODEL_NAME}_patch_embeddings_percentthrumodel_{PERCENT_THRU_MODEL}.pt'
...
#             gt_images_per_concept_test = torch.load(f'{scratch_dir}/GT_Samples/{DATASET_NAME}/gt_samples_per_concept_test_inputsize_{INPUT_IMAGE_SIZE}.pt')
            ...
            # compute_signed_distances_per_concept(embeds, 
            #                                     concepts, 
            #                                     DATASET_NAME,
            #                                     DEVICE,
            #                                     DISTS_FILE,
            ...
            compute_signed_distances_streaming_multi(embeds, concepts, 
                                                            DATASET_NAME,
                                                            DEVICE,
                                                            DISTS_FILE, 
                                                            scratch_dir, 
...
#             #                                                     con_label=CON_LABEL)
            ...
            #                                    cos_sims, DATASET_NAME, INPUT_IMAGE_SIZE, DEVICE, 
            #                                    CON_LABEL, DISTS_FILE, scratch_dir=scratch_dir,       
            #                                    cluster_batch_size=10, sample_type='patch', patch_size=14)
#             del cos_sims
