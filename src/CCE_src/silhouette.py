...
from CCE_src.utils import compute_cosine_similarity_matrix
...
class Silhouette:
    ...
    def _intra_cluster_distances_block(
        ...
        for label, values_ in zip(unique_labels, values):
            #intra_dist[torch.where(labels == label)[0]] = values_.to(labels.device)
            indices = torch.where(labels == label)[0].to(labels.device)  # Ensure indices are on the same device
            values_ = values_.to(labels.device)  # Move values_ to the same device as labels
            intra_dist[indices] = values_  # Now, assignment should work
    ...
    def _nearest_cluster_distance_block(
        ...
        for (label_a, label_b), (values_a, values_b) in zip(label_combinations, values):
            ...
            indices_a = torch.where(labels == label_a)[0].to(labels.device)  # Move indices to same device as labels
            values_a = values_a.to(inter_dist.device)  # Move values_a to same device as inter_dist
            inter_dist[indices_a] = torch.minimum(values_a, inter_dist[indices_a])

            del indices_a
            indices_b = torch.where(labels == label_b)[0].to(labels.device)  # Move indices to same device as labels
            values_b = values_b.to(inter_dist.device)  # Move values_a to same device as inter_dist
