...
from visualize_concepts_w_samples_utils import get_user_category
...
def compute_concept_thresholds(gt_samples_per_concept, embeddings, cos_sims, percentile, n_vectors=5, device="cuda", n_concepts_to_print=0, all_object_patches=None):
    """
...
def evaluate_thresholds_across_dataset(concept_thresholds, gt_samples_per_concept, cos_sims, dataset_name, sample_type, all_object_patches=None):
    """
...
def concept_heatmap(concept_embeddings, con_label, dataset_name='CLEVR', resort=True):
    """
    ...
    concept_embeddings = torch.stack([concept_embeddings[name] for name in concept_names])
    
    # Normalize embeddings to unit norm
    norms = torch.norm(concept_embeddings, p=2, dim=1, keepdim=True)
    normalized_embeddings = concept_embeddings / norms
    
    #compute cosine similarities
    cosine_similarity_matrix = np.array(normalized_embeddings @ normalized_embeddings.T)
...
def concept_heatmap_groupedby_concept(concept_embeddings, con_label, dataset_name='CLEVR'):
    """
...
def concept_heatmap_random_samples(concepts, con_label, num_samples=15, dataset_name='CLEVR'):
    """
    ...
    concept_names = random.sample(list(concepts.keys()), min(num_samples, len(concepts)))
    concept_names.sort()
    
    # Extract embeddings for the sampled concepts
    embeddings = torch.stack([concepts[name] for name in concept_names])
    ...
    cosine_similarity_matrix = torch.matmul(normalized_embeddings, normalized_embeddings.T).cpu().numpy()
...
def compute_cossim_hist_stats(gt_samples_per_concept, cos_sims, dataset_name, percentile, sample_type, all_object_patches=None):
    """
...
def plot_cosine_similarity_histograms(stats, concept_thresholds, sample_type, plot_type="both", percentile=None, bins=50):
    """
    ...
    for i, concept in enumerate(concepts):
        ...
        if plot_type in {"both", "train"}:
            n_out_train, _, _ = ax.hist(out_concept_sims_train, bins=bins, alpha=0.6, color='lightblue', label='Train - Out-of-Concept')
            n_in_train, _, _ = ax2.hist(in_concept_sims_train, bins=bins, alpha=0.6, color='lightcoral', label='Train - In-Concept')

        if plot_type in {"both", "test"}:
            n_out_test, _, _ = ax.hist(out_concept_sims_test, bins=bins, alpha=0.6, color='blue', label='Test - Out-of-Concept')
            n_in_test, _, _ = ax2.hist(in_concept_sims_test, bins=bins, alpha=0.6, color='red', label='Test - In-Concept')
        ...
        handles = []
        if plot_type in {"both", "train"}:
            handles.append(plt.Rectangle((0,0),1,1, color='lightblue', alpha=0.6, label='Train - Out-of-Concept'))
            handles.append(plt.Rectangle((0,0),1,1, color='lightcoral', alpha=0.6, label='Train - In-Concept'))
        if plot_type in {"both", "test"}:
            handles.append(plt.Rectangle((0,0),1,1, color='blue', alpha=0.6, label='Test - Out-of-Concept'))
            handles.append(plt.Rectangle((0,0),1,1, color='red', alpha=0.6, label='Test - In-Concept'))
        if percentile is not None:
            handles.append(plt.Line2D([0], [0], color='green', linestyle='--', linewidth=2, label=f'{percentile:.2f}% Threshold'))
