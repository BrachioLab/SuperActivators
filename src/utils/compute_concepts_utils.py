...
def get_clip_cls_embeddings(model, processor, images, device, percent_thru_model):
    ...
    if percent_thru_model == 100:
        ...
        all_embeddings = get_intermediate_representations(
...
def get_llama_cls_embeddings(model, processor, images, device, percent_thru_model=100):
    ...
    Returns:
        torch.Tensor: The generated embeddings per patch per image.
    """
    inputs = processor(images, add_special_tokens=False, return_tensors="pt").to(device)
...
def compute_raw_batch_embeddings(
    images,
    embedding_fxn,
    model,
    processor,
...
def gpu_kmeans(
    n_clusters, embeddings, dataset_name, device, concepts_filename=None, sample_type="patch"
):
    """
    ...
    train_labels, test_labels, cluster_centers = run_fast_pytorch_kmeans(
        n_clusters, train_embeddings, test_embeddings, device
    )
...
def aggregate_concept_vectors(concept_embeddings, dataset_name, save_file=None):
    ...
    concept_vectors = {
        concept: torch.stack(embeddings).mean(dim=0)
    ...
    global_l2_norm = all_vectors.norm(
        dim=1, keepdim=True
    ).mean()  # Mean L2 norm of all concept vectors
...
def compute_avg_concept_vectors(
    gt_samples_per_concept_train, embeddings, dataset_name=None, output_file=None
):
    """
...
def compute_a_linear_separator(
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch [{epoch+1}/{epochs}] - Train Loss: {avg_train_loss:.6f} - Train Acc: {train_accuracy:.4f} - Train F1: {train_f1:.4f}, Test Loss: {avg_test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}, Test F1: {test_f1:.4f}",
            end="\r",
        )
...
def compute_linear_separators(
    ...
    for concept_name in tqdm(concept_names):
        ...
        linear_separator, concept_logs = compute_a_linear_separator(
            concept_name,
            embeds,
            concept_gt_patches,
            split_df,
...
def compute_cosine_sims(embeddings, concepts, output_file, dataset_name, device, batch_size=32):
    ...
    for i in tqdm(range(0, n_images, batch_size), desc="Processing batches"):
        ...
        cosine_similarities = F.cosine_similarity(
            batch_embeddings.unsqueeze(1), all_concept_embeddings_tensor.unsqueeze(0), dim=2
        )
...
def plot_similar_patches_to_given_patch(
    image_index,
    ...
    patch_idx = get_global_patch_idx(
        image_index,
        patch_index_in_image,
        images,
    ...
    make_image_with_highlighted_patch(
        image,
        left,
        top,
        right,
        bottom,
        plot_image_title=f"Image {image_index}: Patch {patch_index_in_image}",
    )
    ...
    cos_sims = cosine_similarity(
        patch_embedding.unsqueeze(0).cpu().numpy(), embeddings.cpu().numpy()
    ).flatten()
    ...
    overall_title = (
        f"{top_k} Patches Most Similar to Image {image_index}, Patch {patch_index_in_image}"
    )
...
def find_top_k_concepts_for_patch(patch_idx, embeddings, concepts, top_k=5):
    ...
    for concept_label, concept_tensor in concepts.items():
        sim = cosine_similarity(
            patch_embedding.unsqueeze(0).cpu().numpy(), concept_tensor.unsqueeze(0).cpu().numpy()
        ).flatten()
...
def plot_concepts_most_aligned_w_chosen_patch(
    image_index,
    patch_index_in_image,
    images,
    embeddings,
    ...
    top_k_concepts, top_k_sims = find_top_k_concepts_for_patch(
        patch_idx, embeddings, concepts, k_concepts
    )
    ...
    for i, concept_label in enumerate(top_k_concepts):
        ...
        plot_top_patches_for_concept(
            str(concept_label),
            cos_sims,
            images,
            save_path,
            top_n=n_examples_per_concept,
            patch_size=patch_size,
            model_input_size=model_input_size,
        )
...
def evaluate_clustering_metrics(n_clusters_list, embeddings, dataset_name, device):
    ...
    for i, metric in enumerate(metric_names):
        for j, split in enumerate(["Train", "Test"]):
            ...
            (line,) = axes[i].plot(
                [], [], label=label, color=colors[j], marker="o" if j == 0 else "x"
            )
            lines[label] = line
