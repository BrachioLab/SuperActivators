...
def compute_train_avg_and_norm(embeddings, dataset_name, model_input_size):
    #Load split_df
...
def center_and_normalize_embeddings(embeddings, dataset_name, model_input_size):
    """
    ...
    """
    mean_train_embedding, train_norm = compute_train_avg_and_norm(embeddings, dataset_name, model_input_size)
...
def compute_batch_embeddings(images, embedding_fxn, model, processor, device, 
    ...
    torch.save(embeddings, f'Embeddings/{dataset_name}/raw_{embeddings_file}')
    
    norm_embeddings, mean_train_embedding, train_norm = center_and_normalize_embeddings(embeddings, dataset_name, model_input_size)
...
# def run_fast_pytorch_kmeans(n_clusters, train_embeddings, test_embeddings, device='cuda', max_iter=300):
#     """
...
def map_samples_to_clusters(train_image_indices, train_labels, test_image_indices, test_labels, dataset_name, concepts_filename=None):
    ...
    return train_cluster_to_samples, test_cluster_to_samples
...
def gpu_kmeans(n_clusters, embeddings, dataset_name, device, model_input_size, concepts_filename=None, sample_type ='patch', map_samples=True):
    ...
    train_cluster_to_samples, test_cluster_to_samples = [], []
    if map_samples:
        train_cluster_to_samples, test_cluster_to_samples = map_samples_to_clusters(train_relevant_indices, train_labels, 
                                                                            test_relevant_indices, test_labels, dataset_name,
                                                                            concepts_filename)
...
def balance_dataset(embeds, labels, seed=42):
    """
    Balance the dataset for positive and negative examples.

    Args:
        embeds (torch.Tensor): Tensor of embeddings.
        labels (torch.Tensor): Binary labels corresponding to the embeddings.
    ...
    balanced_indices = torch.tensor(list(pos_indices) + list(neg_indices), dtype=torch.long)
...
def write_batch_cosine_sims(writer, concept_keys, cosine_sim_tensor):
    """
    ...
    """
    batch_sims = cosine_sim_tensor.cpu().tolist()
    ...
    with open(output_path, mode='w', newline='') as f:
        ...
        with torch.no_grad():
            for i in tqdm(range(0, embeddings.shape[0], batch_size), desc="Computing cosine similarities"):
                ...
                write_batch_cosine_sims(writer, concept_keys, cosine_similarities)
...
def compute_signed_distances(embeds, cluster_weights, dataset_name, device, output_file, scratch_dir, batch_size=100):
    ...
    weight_matrix = torch.stack([cluster_weights[cid] for cid in cluster_ids]).to(device).to(embeds.dtype)
    weight_matrix = torch.nn.functional.normalize(weight_matrix, dim=1)
        ...
        with torch.no_grad():
            for i in tqdm(range(start_idx, embeds.shape[0], batch_size), desc="Writing signed distances"):
                try:
                    ...
                    break  # safe exit — will resume here next time
...
def evaluate_clustering_metrics(n_clusters_list, embeddings, dataset_name, device, model_input_size, concepts_filenames=None, sample_type='patch'):
    ...
    for i, n_clusters in enumerate(n_clusters_list):
        ...
        cluster_centers = torch.load(f'Concepts/{dataset_name}/cluster_centers_{concepts_filename}')
        train_labels = torch.load(f'Concepts/{dataset_name}/train_labels_{concepts_filename}')
        test_labels = torch.load(f'Concepts/{dataset_name}/test_labels_{concepts_filename}')
