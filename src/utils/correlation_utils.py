...
def get_dynamic_binary_image_concept_activations(cosine_similarities, device, percentile=90):
    ...
    threshold = torch.quantile(
        cosine_similarities.flatten()[:3000], percentile / 100
    )  # Flatten to 1D for quantile calculation
    ...
    for i in range(len(group_vectors)):
        for j in range(i + 1, len(group_vectors)):
            cos_sim = F.cosine_similarity(
                group_vectors[i].unsqueeze(0), group_vectors[j].unsqueeze(0)
            )
...
def compute_group_cosine_sims(group_vectors, embeds, device, batch_size=1024):
    ...
    for i in tqdm(range(0, n_samples, batch_size)):
        ...
        group_vectors_expanded = (
            group_vectors.unsqueeze(0).expand(batch_embeds.size(0), -1, -1).to(device)
        )  # [batch_size, n_groups, embed_dim]

        # Compute cosine similarity for the batch
        batch_cos_sim = F.cosine_similarity(
            batch_embeds.unsqueeze(1).to(device), group_vectors_expanded, dim=2
        )  # [batch_size, n_groups]
...
def plot_aggregated_most_similar_patches_w_heatmaps_and_corr_images(
    ...
    for patch_idx in tqdm(most_similar_patches):
        ...
        image_idx = int(
            patch_idx // ((model_input_size[0] // patch_size) * (model_input_size[1] // patch_size))
        )
...
def plot_aggregated_heatmaps_and_corr_images(
    image_indices,
    group,
    images,
    con_label,
