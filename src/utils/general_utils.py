...
def retrieve_image(img_idx, dataset_name, test_only=False):
    ...
    """
    try:
        metadata = pd.read_csv(f'/scratch/cgoldberg/{dataset_name}/metadata.csv')
        image_path = os.path.join(f'/scratch/cgoldberg/{dataset_name}', metadata.iloc[img_idx]['image_path'])
    except:
...
def load_text(dataset_name):
    ...
    return all_text, train_text, test_text
...
def retrieve_topn_samples(dataset_name, top_n, start_idx=0, split='test'):
    ...
    if split == 'both':
        ...
        my_indices = metadata[metadata['split'] == split].index[start_idx:top_n+start_idx]
    return my_indices
