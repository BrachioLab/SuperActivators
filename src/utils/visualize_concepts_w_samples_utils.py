...
from patch_alignment_utils import (
    calculate_patch_indices,
    calculate_patch_location,
    compute_patch_similarities_to_vector,
    compute_patches_per_image,
    get_image_idx_from_global_patch_idx,
    get_patch_split_df,
)
...
def get_user_category(concept_columns):
    ...
    categories = sorted(
        set(col.split("::")[0] for col in concept_columns)
    )  # Get unique concept categories
    ...
    while True:
        try:
            choice = int(
                input(
                    f"Enter the number of the concept category you want to choose (1-{len(categories)}): "
                )
            )
...
def get_user_concept(category, concept_columns):
    ...
    for idx, concept in enumerate(concepts):
        print(
            f"{idx + 1}. {concept.split('::')[1]}"
        )  # Display the specific concept (e.g., 'red', 'cube', etc.)

    while True:
        try:
            choice = int(
                input(
                    f"Enter the number of the specific concept you want to choose (1-{len(concepts)}): "
                )
            )
...
def plot_aligned_images(
    ...
    plt.suptitle(
        f"Top {k} Images with Highest {metric_type} to: Concept {concept_key}", fontsize=16
    )
...
###### Patch Similarities #####
def neighboring_patch_comparisons(
    image_index,
    patch_index_in_image,
    embeddings,
    images,
    dataset_name="CLEVR",
    patch_size=14,
    ...
    cos_sims = F.cosine_similarity(
        selected_patch_embedding.unsqueeze(0).cpu(), image_patch_embeddings.cpu()
    ).flatten()
...
def make_image_with_highlighted_patch(
    image,
    left,
    top,
    right,
...
def plot_patches_w_corr_images(
    patch_indices,
    concept_cos_sims,
    images,
    overall_title,
    ...
    for i, patch_idx in enumerate(patch_indices):
        ...
        image_idx = get_image_idx_from_global_patch_idx(
            patch_idx, patch_size=patch_size, image_size=model_input_size
        )
        ...
        image_with_patch = make_image_with_highlighted_patch(
            image, left, top, right, bottom, model_input_size
        )
        ...
        axes[1, i].set_title(
            f"Patch {patch_idx} ({metric_type}: {concept_cos_sims[patch_idx]:.2f})"
        )
        axes[1, i].axis("off")
...
def plot_top_patches_for_concept(
    concept_label,
    cos_sims,
    images,
    dataset_name,
...
def plot_most_similar_patches_w_heatmaps_and_corr_images(
    concept_label,
    images,
    cos_sims,
    embeds,
    ...
    most_similar_patches = (
        test_cos_sims[concept_label].sort_values(ascending=False).head(top_n).index
    )
    heatmaps = {}
    images_w_patches = []
    ...
    for i, patch_idx in enumerate(most_similar_patches):
        ...
        axes[1, i].set_title(
            f"Heatmap Max = {round(heatmap.max().item(), 4)}\nHeatmap Min = {round(heatmap.min().item(), 4)}"
        )
        axes[1, i].axis("off")
...
def plot_patchsims_for_concept(
    concept_label,
    heatmaps,
    image_indices,
    images,
    ...
    fig, axes = plt.subplots(
        2, top_n, figsize=(top_n * 3, 6)
    )  # 2 rows: 1 for images, 1 for heatmaps
    ...
    for i, image_index in enumerate(image_indices):
        ...
        heatmap_overlay = ax_heatmap.imshow(
            heatmap,
            cmap="hot",
            alpha=0.6,
            vmin=vmin,
    ...
    if save_file:
        save_path = (
            f"../Figs/{dataset_name}/patch_alignment_to_concept/{concept_label}_allimages.png"
        )
        plt.savefig(save_path, bbox_inches="tight", dpi=500)
...
def plot_patchsims_heatmaps_all_concepts(
    concept_labels,
    heatmaps,
    image_indices,
    images,
    ...
    fig, axes = plt.subplots(
        num_concepts + 1, top_n, figsize=(top_n * 3, (num_concepts + 1) * 3)
    )  # 1 row for images, other rows for heatmaps
    ...
    for i, image_index in enumerate(image_indices):
        ...
        for j, concept_label in enumerate(concept_labels):
            heatmap = heatmaps[concept_label][
                image_index
            ]  # Access heatmap for this concept and image
            ...
            ax_heatmap.imshow(
                resized_image.convert("L"), cmap="gray", alpha=0.4
            )  # Overlay the image in gray
            ax_heatmap.axis("off")
...
def plot_patchsims_all_concepts(
    img_idx,
    heatmaps,
    vmin=None,
    vmax=None,
...
def plot_patches_sim_to_vector(
    cos_sim_grid,
    resized_image,
    patch_size,
    image_index,
    ...
    heatmap = ax.imshow(
        cos_sim_grid, cmap="hot", alpha=0.5, extent=(0, image_width, image_height, 0)
    )

    # Highlight the selected patch
    if patch_index_in_image >= 0:
        rect = plt.Rectangle(
            (left, top), patch_size, patch_size, edgecolor="red", facecolor="none", linewidth=2
        )
        ax.add_patch(rect)
...
def binarize_patchsims_for_concept(
    concept_label,
    threshold,
    heatmaps,
    image_indices,
    ...
    for i, image_index in enumerate(image_indices):
        ...
        rgba_image[mask_resized] = np.concatenate(
            [image_np[mask_resized], np.full((mask_resized.sum(), 1), 1)], axis=1
        )
        ...
        axes[1, i].set_title("Binarized Heatmap")
        axes[1, i].axis("off")

    plt.suptitle(
        f"Concept {concept_label} Binarized {metric_type} (Th={threshold:.5f})", fontsize=16, y=1.05
    )
    plt.tight_layout(rect=[0, 0, 1, 1])

    if save_file:
        save_path = (
            f"../Figs/{dataset_name}/patch_alignment_to_concept/{concept_label}_binarized.png"
        )
        plt.savefig(save_path, bbox_inches="tight", dpi=500)
...
def plot_binarized_patchsims_all_concepts(
    concept_labels,
    heatmaps,
    image_indices,
    images,
    ...
    for j, concept_label in enumerate(concept_labels):
        axes[j + 1, 0].text(
            -0.1,
            0.5,
            f"{concept_label}\n(thr = {thresholds[concept_label][0]:.5f})",
            fontsize=20,
            verticalalignment="center",
            horizontalalignment="right",
            transform=axes[j + 1, 0].transAxes,
    ...
    for i, image_index in enumerate(image_indices):
        for j, concept_label in enumerate(concept_labels):
            ...
            mask_resized = Image.fromarray((mask * 255).astype(np.uint8)).resize(
                model_input_size, resample=Image.NEAREST
            )
            mask_resized = np.array(mask_resized) > 127
