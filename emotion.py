...
def label_words_with_emotion(words, model=None):
    """
    Given a list of words, return a list of dicts with valence/arousal scores.
    """
    if model is None:
    ...
    emotions = load_emotions()
    axis_labels = ["NV", "PV", "HA", "LA"]
    axis_points = [np.mean(model.encode(emotions[label]), axis=0) for label in axis_labels]
    x1, x2, y1, y2 = axis_points[0], axis_points[1], axis_points[3], axis_points[2]

    embeddings = model.encode(words)
    projections = project_points_onto_axes(embeddings, x1, x2, y1, y2)
    valences = projections[0]
    arousals = projections[1]

    return [{"word": w, "valence": v, "arousal": a} for w, v, a in zip(words, valences, arousals)]