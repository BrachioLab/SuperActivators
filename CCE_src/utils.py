...
def compute_cosine_similarity_matrix(data1, data2, batch_size=1024):
    data1_c = torch.where(
        torch.logical_and(data1 > 0, data1 < 1e-4), torch.full_like(data1, 1e-4), data1
    )
    data2_c = torch.where(
    ...
    norms_data1 = torch.norm(data1_c, dim=1, keepdim=True)
    norms_data2 = torch.norm(data2_c, dim=1, keepdim=True)

    sim_matrix = torch.zeros((data1.size(0), data2.size(0)), device='cpu')

    for i in range(0, data1.size(0), batch_size):
        data1_batch = data1_c[i : i + batch_size]
        norms1_batch = norms_data1[i : i + batch_size]

        sim_matrix[i : i + batch_size] = (torch.mm(data1_batch, data2_c.T) / (
            norms1_batch * norms_data2.T + 1e-4)).to(data1.device)
...
def kpp(data: torch.Tensor, k: int, sample_size: int = -1, cosine=False, dot=False):
    ...
    for i in range(k):
        if i == 0:
            ...
            # ]
            sample = r.sample([1]).to(cumprobs.device)  # Ensure sample is on the same device as cumprobs
            init[i, :] = data[torch.searchsorted(cumprobs, sample)]
