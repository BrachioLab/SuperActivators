...
from CCE_src import utils
from CCE_src.nmf import learn_seminmf_2
from CCE_src.silhouette import Silhouette
from CCE_src.utils import Patch
...
def learn_split_subspace(
    ...
    mult = 0.1
    init = None
    for j in tqdm(range(epochs), desc="Epoch"):
...
def get_sam_segments(images):
    ...
    for out in generator(ListDataset(images), points_per_batch=32):
...
class ConceptLearner:
    ...
    def learn_concepts(
        ...
        return concepts, patches, patch_embf
    ...
    def learn_attribute_concepts(
        ...
        for cid in tqdm(range(n_attributes), desc="Learning Attribute Subspace"):
        #for cid in range(n_attributes):
            #print(f"cid {cid} / {n_attributes} attributes")
