...
def compute_concept_thresholds(gt_samples_per_concept, cos_sims, percentile, device, dataset_name, con_label, n_vectors=5,  n_concepts_to_print=0):
    """
    ...
    #     # if set(concept_thresholds.keys()) != set(gt_samples_per_concept.keys()):
    #     #         concept_thresholds = {k:v for k,v in concept_thresholds if k in gt_samples_per_concept.keys()}
...
def compute_concept_thresholds_over_percentiles(gt_samples_per_concept, cos_sims, percentiles, device, dataset_name, con_label, n_vectors=5, n_concepts_to_print=0):
    """
...
#     except FileNotFoundError:
    ...
    if new_percentiles:
        ...
        cos_sims_tensor = torch.tensor(cos_sims.values.astype(np.float32), device=device)
        concept_names = list(cos_sims.columns)
        sims_list = []
...
def evaluate_baseline_models_across_dataset(gt_samples_per_concept, dataset_name, sample_type, model_input_size, 
    ...
    # If all_object_patches is provided, restrict to these indices.
    if all_object_patches is not None:
        relevant_indices = torch.tensor([int(idx.item()) for idx in relevant_indices if int(idx.item()) in all_object_patches])
    ...
    for concept, concept_labels in all_concept_labels.items():
        ...
        for trial in range(n_trials):
            ...
            if balance_dataset:
                # Balance the dataset by sampling equally from positives and negatives.
                pos_positions = torch.where(relevant_labels == 1)[0]
                neg_positions = torch.where(relevant_labels == 0)[0]
                min_count = min(len(pos_positions), len(neg_positions))
                if min_count > 0:
                    ...
                    balanced_positions = torch.cat([sampled_pos, sampled_neg])
                    balanced_positions, _ = torch.sort(balanced_positions)
                    relevant_indices_bal = relevant_indices[balanced_positions]
                    relevant_labels = concept_labels[relevant_indices_bal]
                else:
                    # If one group is empty, continue with original labels.
                    pass
...
def compute_concept_metrics(fp_count, fn_count, tp_count, tn_count, concepts, dataset_name, con_label, invert_percentile, just_obj=False, baseline_type=None, detect_percentile=None):
...
def plot_cosine_similarity_histograms(stats, concept_thresholds, sample_type, plot_type="both", metric_type='Cosine Similarity', percentile=None, bins=50, concepts=None):
    """
    ...
    fig, axes = plt.subplots(nrows=num_concepts, figsize=(8, 3 * num_concepts), sharex=True)
    if num_concepts == 1:
        axes = [axes]
    ...
    for i, concept in enumerate(concepts):
        ...
        if plot_type in {"both", "train"}:
            ax.hist(out_concept_sims_train, bins=bins, alpha=0.5, color='lightblue', label='Train - Out-of-Concept', density=True)
            ax.hist(in_concept_sims_train, bins=bins, alpha=0.5, color='lightcoral', label='Train - In-Concept', density=True)

        if plot_type in {"both", "test"}:
            ax.hist(out_concept_sims_test, bins=bins, alpha=0.5, color='blue', label='Test - Out-of-Concept', density=True)
            ax.hist(in_concept_sims_test, bins=bins, alpha=0.5, color='red', label='Test - In-Concept', density=True)

        # Set labels and title
        ax.set_xlabel(metric_type)
        ax.set_ylabel(f'{sample_type.capitalize()} {"Density"}')
        ax.set_title(f'Distribution for {concept}')
...
def find_activated_sentences_bytoken(act_metrics, curr_thresholds, model_input_size, dataset_name):
    ...
    threshold_tensor = torch.tensor([thresholds[c] for c in act_metrics.columns])
    ...
    # Compute max activations per sentence
    max_activations = torch.zeros((len(tokens_list), len(act_metrics.columns)))
    for sent_idx, (start, end) in enumerate(sentence_boundaries):
        max_activations[sent_idx] = torch.max(metrics_tensor[start:end], dim=0)[0]
    
    # Compare with thresholds
    activated = max_activations >= threshold_tensor
    
    # Split by train/test
    train_mask = torch.tensor([split_df[i] == 'train' for i in range(len(tokens_list))])
    test_mask = torch.tensor([split_df[i] == 'test' for i in range(len(tokens_list))])
...
def get_patch_detection_tensor(act_metrics, detect_thresholds, model_input_size, dataset_name, patch_size=14):
    ...
    if model_input_size[0] == 'text':
        ...
        for sent_idx, tokens in enumerate(tokens_list):
            token_to_sentence_idx.extend([sent_idx] * len(tokens))
        sample_indices = torch.tensor(token_to_sentence_idx)
    else:
...
def compute_detection_metrics_for_per(per, gt_images_per_concept_test, 
                                      activated_images_test, relevant_indices, 
    ...
    Args:
        ...
        relevant_indices: Set of relevant image indices
...
def compute_detection_metrics_over_percentiles(percentiles, gt_samples_per_concept_test, gt_images_per_concept_test, 
    ...
    for per in tqdm(percentiles):
        ...
        curr_thresholds = compute_concept_thresholds(
            ...
            device=device, dataset_name=dataset_name, con_label=con_label,
        ...
        metrics_df = compute_detection_metrics_for_per(
            ...
            relevant_indices, dataset_name, con_label
        )
...
def plot_activation_percentages_over_thresholds(cos_sims, gt_patches_per_concept_train, 
    ...
    for percentile in tqdm(in_concept_patch_thresholds):
        try: #just load if you already did this computation
            pct_train_in = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_train_inconcept_{con_label}.pt')
            pct_test_in = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_test_inconcept_{con_label}.pt')
            pct_train_out = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_train_outconcept_{con_label}.pt')
            pct_test_out = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_test_outconcept_{con_label}.pt')
        except:
...
def plot_activation_count_distributions(cos_sims, gt_patches_per_concept_train, 
    ...
    for percentile in tqdm(threshold_percentiles):
        try:
            ...
            activated_counts_train = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_activated_counts_train_{con_label}.pt')
            activated_counts_test  = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_activated_counts_test_{con_label}.pt')
        except:
            curr_thresholds = compute_concept_thresholds(gt_patches_per_concept_test,  
                                                         cos_sims, percentile, device=device, 
                                                         dataset_name=dataset_name, con_label=con_label,
...
def compute_multiple_activation_analyses(cos_sims, gt_patches_per_concept_train, gt_patches_per_concept_test, gt_images_per_concept_train, gt_images_per_concept_test, model_input_size, dataset_name, con_label, device):
    try:
        gt_activated_counts_train = torch.load(f'Quant_Results/{dataset_name}/gt_activated_counts_train_{con_label}.pt')
        gt_activated_counts_test = torch.load(f'Quant_Results/{dataset_name}/gt_activated_counts_test_{con_label}.pt')
    except:
    ...
    for percentile in tqdm([0.02, 0.05, 0.07, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 1.0]):
        try:
            activated_counts_train_inconcept = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_activated_counts_train_inconcept_{con_label}.pt')   
            activated_counts_test_inconcept = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_activated_counts_test_inconcept_{con_label}.pt')
            activated_counts_train_outconcept = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_activated_counts_train_outconcept_{con_label}.pt')
            activated_counts_test_outconcept = torch.load(f'Quant_Results/{dataset_name}/percentile_{percentile}_activated_counts_test_outconcept_{con_label}.pt')
        except:
            curr_thresholds = compute_concept_thresholds(gt_patches_per_concept_test,  
                                                         cos_sims, percentile, device=device, 
                                                         dataset_name=dataset_name, con_label=con_label,
