...
def compute_concept_thresholds_over_percentiles_all_pairs(cos_sims_df, gt_samples_per_concept, percentiles, device,
    ...
    for concept, indices in tqdm(gt_samples_per_concept.items(), desc="Concepts"):
        if len(indices) == 0:
            continue
...
def plot_weighted_detection_curve_for_best_clusters(best_cluster_per_concept, 
    ...
    if model_name == 'CLIP':
        gt_samples_per_concept = torch.load(f'GT_Samples/{dataset_name}/gt_samples_per_concept_test_inputsize_(224, 224).pt')
    elif model_name == 'Llama':
        gt_samples_per_concept = torch.load(f'GT_Samples/{dataset_name}/gt_samples_per_concept_test_inputsize_(560, 560).pt')
    else:
...
def filter_and_save_best_clusters(dataset_name, con_label):
    ...
    best_clusters_by_detect = torch.load(f'Unsupervised_Matches/{dataset_name}/bestdetects_{con_label}.pt')
...
def get_matched_concepts_and_data(
    ...
    matches_acts, matched_gt_test, matched_gt, matched_concepts = None, None, None, None
