...
def get_sentence_category(sentence_idx, dataset_name):
    ...
    return metadata['class'].iloc[sentence_idx]
...
def plot_tokens_in_context_byconcept(cos_sims, tokens_list, dataset_name, concept=None, top_k=5, top=True, aggr_method='avg', cmap_name="coolwarm"):
    if concept is None:
        concept = user_select_concept(cos_sims.columns)

    sentences = get_sentences_by_metric(cos_sims, tokens_list, dataset_name, concept, top_k, top, aggr_method) 
    # for sentence in sentences:
    #     print(tokens_list)

    if top:
        print(f"\nPlotting sentences MOST activated by {concept} ({aggr_method} over tokens)\n")
    else:
        print(f"\nPlotting sentences LEAST activated by {concept} ({aggr_method} over tokens)\n")

    all_scores = []
    for sentence_idx, _ in sentences:
        start_idx, end_idx = get_glob_tok_indices_from_sent_idx(sentence_idx, tokens_list)
        all_scores.extend(cos_sims[concept].iloc[start_idx:end_idx].tolist())
    vmin, vmax = min(all_scores), max(all_scores)

    html_blocks = []
    for i, (sentence_idx, metric) in enumerate(sentences):
        ...
        # Title for the sentence with metadata
        title = f"<h4>Rank {i+1} : Sentence {sentence_idx} -- {sentence_class.capitalize()} ({aggr_method}={metric:.2f})</h4>"

        # Render sentence with no print(), just HTML
...
def plot_all_concept_activations_on_sentence(sentence_idx, cos_sims, tokens_list, dataset_name, cmap_name="coolwarm"):
    concepts = cos_sims.columns.tolist()
    tokens = tokens_list[sentence_idx]
    start_idx, end_idx = get_glob_tok_indices_from_sent_idx(sentence_idx, tokens_list)

    # Get all per-concept scores
    sims_per_concept = {
    ...
    vmin, vmax = min(all_sims), max(all_sims)
    ...
    for concept in concepts:
        ...
        concept_header = f"<h4 style='margin-bottom: 4px;'>Concept: {concept} (min={min(sims):2f}, max={max(sims):2f}, avg={np.mean(sims):2f}) </h4>"
